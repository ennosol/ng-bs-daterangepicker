...
Using bower:
```
bower install nsl-bootstrap-daterangepicker
```

Using npm:
```
npm install nsl-bootstrap-daterangepicker
```
...
```html
<script src="nsl-bootstrap-daterangepicker.js"></script>
```

Then, inject `nslBootstrapDaterangepicker` in your application module:

```js
angular.module('myApp', ['nslBootstrapDaterangepicker']);
```

and then just add an `input` of type `daterange`:

```html
<input nsl-bootstrap-daterangepicker type="text" ng-model="myDateRange" />
```
...
[![Bitdeli Badge](https://d2weczhvl823v0.cloudfront.net/luisfarzati/nsl-bootstrap-daterangepicker/trend.png)](https://bitdeli.com/free "Bitdeli Badge")
